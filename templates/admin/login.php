...
<head>
    ...
    <style>
        ...
        .btn { background: #1A237E; color: #fff; border: 0; border-radius: 8px; padding: 11px 16px; cursor: pointer; font-size: 1rem; text-decoration: none; display: inline-block; }
        .erro { background: #fff5f5; border: 1px solid #ef9a9a; color: #c62828; padding: 14px 16px; border-radius: 8px; margin-bottom: 16px; }
        label { font-weight: bold; }
        .nota { color: #64748b; font-size: 0.95rem; margin-top: 8px; }
    </style>
</head>
<body>
    <div class="card">
        <h1><?= htmlspecialchars($titulo ?? 'Administração') ?></h1>

        <?php if (!empty($erro)): ?>
            <div class="erro"><?= htmlspecialchars($erro) ?></div>
        <?php endif ?>

        <form method="POST" action="<?= htmlspecialchars(app_url('admin/login')) ?>">
        ...
        <div class="nota">Credenciais padrão configuradas no `.env`: `admin@autoshop.pt` / `password`.</div>
    </div>
</body>
</html>
